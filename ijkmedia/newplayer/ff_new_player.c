...
static double compute_target_delay(NewFFPlayer *ffp, double delay, PlayerState *is)
{
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len){
    ...
    if (!isnan(is->audio_clock)) {
        set_clock_at(&is->audclk, is->audio_clock - (double)(is->audio_write_buf_size) / is->audio_tgt.bytes_per_sec - SDL_AoutGetLatencySeconds(ffp->aout), 0, audio_callback_time / 1000000.0);
...
static int get_video_frame(NewFFPlayer *ffp, AVFrame *frame)
{
    PlayerState *is = ffp->videoState;
    int got_picture;
